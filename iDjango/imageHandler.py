...
def add_default_image():
    some_data = open('image/favicon.ico', 'rb').read()
    img = create_image_dict(data = some_data, fileName = "fav.ico",\
                                       description = "Favorite icon")
    add_image(img)
    commentForm = {'i': 0, 'user': 'Minh', 'comment': 'Small file for testing'}
    add_comment(commentForm)
    
    some_data = open('image/dice.png', 'rb').read()
    img = create_image_dict(data = some_data, fileName = "dice.png",\
                                       description = "Four colorful dices")
    add_image(img)
    commentForm = {'i': 1, 'user': 'Minh', 'comment': 'First comment ever'}
    add_comment(commentForm)

    some_data = open('image/tux.png', 'rb').read()
    img = create_image_dict(data = some_data, fileName = "tux.png",\
